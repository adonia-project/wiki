#!/usr/bin/env python3
"""
TALOD Wiki Sync Bot — Guajicutea Article Synchronizer

Two-way sync between local .mediawiki files and the TALOD Miraheze wiki.
Only syncs articles under articles/Countries/Guajicutea/.

Usage:
    python sync_guajicutea.py              # Full sync (push + pull)
    python sync_guajicutea.py --push       # Push local changes to wiki only
    python sync_guajicutea.py --pull       # Pull wiki changes to local only
    python sync_guajicutea.py --status     # Show what would change (dry run)
    python sync_guajicutea.py --list       # List all tracked articles
"""
...
SYNC_STATE_FILE = REPO_ROOT / ".talod-sync-state-guajicutea.json"
...
def do_status(site, state):
    ...
    log(f"Found {len(local_files)} local Guajicutea articles")
...
def do_list():
    """List all tracked Guajicutea articles."""
    local_files = get_local_files()
    log(f"{len(local_files)} Guajicutea articles found:")
...
def main():
    parser = argparse.ArgumentParser(description="TALOD Wiki Sync Bot for Guajicutea articles")

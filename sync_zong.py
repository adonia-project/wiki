#!/usr/bin/env python3
"""
TALOD Wiki Sync Bot — Zong Article Synchronizer

Two-way sync between local .mediawiki files and the TALOD Miraheze wiki.
Only syncs articles under articles/Countries/Zong/.

Usage:
    python sync_zong.py              # Full sync (push + pull)
    python sync_zong.py --push       # Push local changes to wiki only
    python sync_zong.py --pull       # Pull wiki changes to local only
    python sync_zong.py --status     # Show what would change (dry run)
    python sync_zong.py --list       # List all tracked articles
"""
...
SYNC_STATE_FILE = REPO_ROOT / ".talod-sync-state-zong.json"
...
ZONG_DIR = REPO_ROOT / os.environ.get("ZONG_DIR", "articles/Countries/Zong")
EXTRA_ARTICLE_DIRS = []
...
def get_local_files():
    """Find all .mediawiki files under the Zong directory and extra country dirs."""
    files = {}
    for root in [ZONG_DIR] + EXTRA_ARTICLE_DIRS:
        if not root.exists():
            continue
...
def do_status(site, state):
    ...
    log(f"Found {len(local_files)} local Zong articles")
...
def do_list():
    """List all tracked Zong articles."""
    local_files = get_local_files()
    log(f"{len(local_files)} Zong articles found:")
...
def main():
    parser = argparse.ArgumentParser(description="TALOD Wiki Sync Bot for Zong articles")

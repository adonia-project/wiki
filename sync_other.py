...
Usage:
    ...
    python sync_other.py --push       # Push local changes to wiki only
    python sync_other.py --pull       # Pull wiki changes to local only
    python sync_other.py --status     # Show what would change (dry run)
    python sync_other.py --list       # List all tracked articles
"""
...
SYNC_STATE_FILE = REPO_ROOT / ".talod-sync-state-other.json"
...
def get_local_files():
    """Find all .mediawiki files under the Other directory."""
    files = {}
...
def do_status(site, state):
    ...
    log(f"Found {len(local_files)} local Other articles")
...
def do_list():
    """List all tracked Other articles."""
    local_files = get_local_files()
    log(f"{len(local_files)} Other articles found:")
...
def main():
    parser = argparse.ArgumentParser(description="TALOD Wiki Sync Bot for Other articles")

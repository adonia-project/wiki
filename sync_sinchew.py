#!/usr/bin/env python3
"""
TALOD Wiki Sync Bot — Sinchew Article Synchronizer

Two-way sync between local .mediawiki files and the TALOD Miraheze wiki.
Only syncs articles under articles/Countries/Sinchew/.

Usage:
    python sync_sinchew.py              # Full sync (push + pull)
    python sync_sinchew.py --push       # Push local changes to wiki only
    python sync_sinchew.py --pull       # Pull wiki changes to local only
    python sync_sinchew.py --status     # Show what would change (dry run)
    python sync_sinchew.py --list       # List all tracked articles
"""
...
SYNC_STATE_FILE = REPO_ROOT / ".talod-sync-state-sinchew.json"
...
SINCHEW_DIR = REPO_ROOT / os.environ.get("SINCHEW_DIR", "articles/Countries/Sinchew")
EXTRA_ARTICLE_DIRS = [REPO_ROOT / "articles/Countries/Louyang"]
...
def get_local_files():
    """Find all .mediawiki files under the Sinchew directory and extra country dirs."""
    files = {}
    for root in [SINCHEW_DIR] + EXTRA_ARTICLE_DIRS:
        if not root.exists():
            continue
...
def do_status(site, state):
    ...
    log(f"Found {len(local_files)} local Sinchew articles")
...
def do_push(site, state, page_names=None, force=False):
    ...
    if page_names is None:
        ...
        to_push = to_push + new_pages          # conflicts are deliberately NOT pushed
    else:
        # explicit page list: still refuse anything that has diverged, unless --force-push
        _, _, conflicts_now, _ = do_status(site, state)
        blocked = [p for p in page_names if p in conflicts_now]
        if blocked and not force:
            log("These pages have diverged from the wiki and would lose edits:", "ERROR")
            for p in blocked: log(f"    {p}", "ERROR")
            log("  Resolve them first, or pass --force-push deliberately.", "ERROR")
            return
        to_push = page_names
...
def do_sync(site, state):
    ...
    if all_to_push:
        do_push(site, state, all_to_push, force=True)

    if not to_pull and not all_to_push and not conflicts:
        log("Everything is in sync.")

def do_list():
    """List all tracked Sinchew articles."""
    local_files = get_local_files()
    log(f"{len(local_files)} Sinchew articles found:")
...
def main():
    parser = argparse.ArgumentParser(description="TALOD Wiki Sync Bot for Sinchew articles")
    ...
    if args.status:
        ...
        if args.force_push in local_files:
            do_push(site, state, [args.force_push], force=True)
        else:

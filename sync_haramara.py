...
Only syncs articles under articles/Countries/Haramara/.

Usage:
    python sync_haramara.py              # Full sync (push + pull)
    python sync_haramara.py --push       # Push local changes to wiki only
    python sync_haramara.py --pull       # Pull wiki changes to local only
    python sync_haramara.py --status     # Show what would change (dry run)
    python sync_haramara.py --list       # List all tracked articles
"""
...
HARAMARA_DIR = REPO_ROOT / os.environ.get("HARAMARA_DIR", "articles/Countries/Haramara")
...
def get_local_files():
    ...
    files = {}
    for path in HARAMARA_DIR.rglob("*.mediawiki"):
...
def get_wiki_page_names(site):
    """Get all pages on the wiki that correspond to our local Haramara articles."""
...
def do_status(site, state):
    ...
    local_files = get_local_files()
    log(f"Found {len(local_files)} local Haramara articles")
...
def do_list():
    ...
    local_files = get_local_files()
    log(f"{len(local_files)} Haramara articles found:")
